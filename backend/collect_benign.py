#!/usr/bin/env python3
"""
Collect benign process telemetry from this machine and save it as training data.

Output: backend/datasets/otrf/benign/collected_YYYYMMDD_HHMMSS.json
Format: Sysmon-compatible NDJSON (same as OTRF datasets — train_otrf.py reads it directly)
...
OUTPUT_DIR = Path(__file__).parent / "datasets" / "otrf" / "benign"
...
def _net_by_pid() -> dict[int, list[tuple]]:
    """Return {pid: [(ip, port, proto), ...]} from current connections."""
...
def _snapshot() -> tuple[list[dict], list[dict]]:
    """Return (process_events, network_events) in Sysmon Event ID 1/3 format."""
    snapshots = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect benign process telemetry for ML training")
    ...
    seen: set[tuple] = set()   # (pid, image, cmdline) — skip re-logging identical process entries
    total_proc     = 0
    total_net      = 0
    snapshots_done = 0
    ...
    print(f"  → {out_path}")
    print(f"\nNext step:  python train_otrf.py")
