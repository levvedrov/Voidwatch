...
ML_ALERT_THRESHOLD = 0.40  # ML score that always triggers an alert
...
def _build_allowlist_sets(db) -> tuple[set[str], set[str], list[str]]:
    """Return (process_names, hashes, cmdline_patterns) from the allowlist table."""
    try:
        from database import Allowlist
        entries = db.query(Allowlist).all()
    except Exception:
        return set(), set(), []
    names    = {e.value.lower() for e in entries if e.kind == "process_name"}
    hashes   = {e.value.lower() for e in entries if e.kind == "hash"}
    patterns = [e.value       for e in entries if e.kind == "cmdline"]
    return names, hashes, patterns


def _is_allowlisted(proc: ProcessData, names: set[str], hashes: set[str], patterns: list[str]) -> bool:
    if proc.name.lower() in names:
        return True
    if proc.sha256 and proc.sha256.lower() in hashes:
        return True
